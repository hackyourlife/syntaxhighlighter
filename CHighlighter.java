...
public class CHighlighter extends Highlighter {
	...
	public String formatLine(String line) {
		...
		while(i < length) {
			...
			} else if(isCPPKeyword(tmp) && !inString && !inCharacter && !inComment) {
				formatted.append("<span class=\"cpp\">" + htmlspecialchars(tmp) + "</span>");
			} else if(isNumeric(tmp) && !inString && !inCharacter && !inComment) {
				formatted.append("<span class=\"number\">" + htmlspecialchars(tmp) + "</span>");
			} else {
				formatted.append(htmlspecialchars(tmp));
			}

			// because the last character read in the while-loop is not part of tmp
			i++;
			...
			// append last character (not contained in tmp) if it was not processed elsewhere
	...
	public static void main(String[] args) throws IOException {
		CHighlighter highlighter = new CHighlighter();
		FileReader src = new FileReader("ifloop.c");
		FileWriter dst = new FileWriter("ifloop.html");
